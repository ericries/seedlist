---
firms:
- round: series-a
  slug: andreessen-horowitz
  year: 2024
- round: series-a
  slug: thrive-capital
  year: 2024
- round: series-b
  slug: thrive-capital
  year: 2024
- round: series-c
  slug: thrive-capital
  year: 2025
- round: series-c
  slug: andreessen-horowitz
  year: 2025
- round: series-c
  slug: dst-global
  year: 2025
- round: series-d
  slug: coatue-management
  year: 2025
- round: Seed
  slug: stripe
  year: 2024
- round: —
  slug: hanabi-capital
  year: 2025
- round: '2025'
  slug: accel
  year: 2025
- round: Angel
  slug: first-harmonic
  year: 2023
- round: Series B
  slug: benchmark
  year: 2024
- round: Seed ($8M round)
  slug: pear-vc
  year: 2023
- round: —
  slug: index-ventures
  year: 2025
founded: 2022
founders:
- name: Michael Truell
  role: CEO & Co-Founder
- name: Sualeh Asif
  role: Co-Founder
- name: Arvid Lunnemark
  role: Co-Founder
- name: Aman Sanger
  role: Co-Founder
investors:
- round: series-a
  slug: patrick-collison
  year: 2024
- round: Series B ($105M at $2.5B, co-lead)
  slug: josh-kushner
  year: 2024
- round: '2025'
  slug: philippe-laffont
  year: 2025
- round: Series B ($105M)
  slug: miles-grimshaw
  year: 2025
- round: —
  slug: mike-volpi
  year: 2025
- round: '2025'
  slug: andrew-braccia
  year: 2025
- round: Angel
  slug: ali-rowghani
  year: 2023
- round: Series B
  slug: sarah-tavel
  year: 2024
- round: Seed ($8M round)
  slug: arash-ferdowsi
  year: 2023
- round: ~2023
  slug: martin-casado
  year: 2023
last_researched: 2026-03-19
location: San Francisco, California
name: Cursor (Anysphere)
sector:
- AI
- developer-tools
- code-editor
slug: cursor
stage_latest: Series D
status: published
status_company: active
total_raised: $3.5B
type: startup
website: https://cursor.com
---

## About

Cursor is an AI-powered code editor built by Anysphere, founded in 2022 by Michael Truell, Sualeh Asif, Arvid Lunnemark, and Aman Sanger while students at MIT [^1]. The product is a fork of Visual Studio Code with integrated AI-assisted coding capabilities and is used by over 50% of the Fortune 500 [^2]. Bloomberg has described it as "the fastest growing startup ever" [^3].

## Funding History

| Date | Round | Amount | Lead | Co-investors |
|------|-------|--------|------|-------------|
| Apr 2022 | Pre-Seed | $400K | Neo | — [^4] |
| Oct 2023 | Seed | $8M | OpenAI Startup Fund | Nat Friedman, Arash Ferdowsi [^1] |
| Aug 2024 | Series A | $60M | a16z, Thrive Capital (co-lead) | Patrick Collison [^5] |
| Dec 2024 | Series B | $105M | Thrive Capital | — [^3] |
| Jun 2025 | Series C | $900M | Thrive Capital | Andreessen Horowitz, Accel, DST Global [^3] |
| Nov 2025 | Series D | $2.3B | Accel, Coatue (co-lead) | Thrive Capital, Andreessen Horowitz, DST Global, NVIDIA, Google [^2] |

Total raised: approximately $3.5B. Valued at $29.3B as of Series D [^2].

## What Investors Say

No verified investor quotes found at this time.

## What Founders Say

No independently sourced founder testimonials found.

## Sources

[^1]: TechCrunch, "Anysphere raises $8M from OpenAI to build an AI-powered IDE," October 11, 2023, accessed March 2026. https://techcrunch.com/2023/10/11/anysphere-raises-8m-from-openai-to-build-an-ai-powered-ide/
[^2]: TechFundingNews, "Anysphere's Cursor soars to $29B valuation with $2.3B round led by Accel, Coatue," November 14, 2025, accessed March 2026. https://techfundingnews.com/anysphere-soars-to-29-3b-valuation-with-2-3b-funding-redefining-the-future-of-coding/
[^3]: Crunchbase News, "AI-Powered Coding Tool Anysphere Raises $900M at $9.9B Valuation," June 2025, accessed March 2026. https://news.crunchbase.com/ai/anysphere-cursor-venture-funding-thrive/
[^4]: Contrary Research, "Cursor Business Breakdown & Founding Story," accessed March 2026. https://research.contrary.com/company/cursor
[^5]: TechCrunch, "Anysphere, a GitHub Copilot rival, has raised an over $60M Series A at $400M valuation from a16z, Thrive, sources say," August 9, 2024, accessed March 2026. https://techcrunch.com/2024/08/09/anysphere-a-github-copilot-rival-has-raised-60m-series-a-at-400m-valuation-from-a16z-thrive-sources-say/