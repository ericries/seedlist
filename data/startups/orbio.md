---
name: "Orbio"
slug: orbio
type: startup
website: "https://orbio.com"
founded: 2025
status_company: active
sector: [AI, hr-tech, future-of-work]
stage_latest: "Series A"
total_raised: "$26M"
investors: []
firms: []
founders:
  - name: "Sergi Bastardas"
    role: "Co-Founder"
  - name: "Nacho Travesí"
    role: "Co-Founder"
  - name: "Antonio Melé"
    role: "Co-Founder"
status: published
last_researched: 2026-06-15
---

## About

Orbio is an AI-powered workforce management startup founded in 2025 by Sergi Bastardas, Nacho Travesí, and Antonio Melé that helps businesses recruit, onboard, manage, and retain frontline employees using AI agents [^1][^2]. The platform deploys AI agents named Maria, Daniel, and Claire that conduct candidate interviews, assess fit, monitor employee output, and handle daily check-ins across the employee lifecycle [^1][^2]. Customers include Poke, YUM! Brands (Pizza Hut, Taco Bell, KFC), and The Stepping Stones Group [^1].

*Disclaimer: This is a lean profile built from initial round coverage and may be expanded as more information becomes available.*

## Funding History

| Date | Round | Amount | Lead | Co-investors |
|------|-------|--------|------|--------------|
| 2026-06-14 | Series A | $21M | Dawn Capital | Visionaries, 2100 Ventures [^1][^2][^3] |

Orbio has raised $26 million in total funding to date [^1][^2].

## What Investors Say

No independently sourced investor commentary verified at this time.

## What Founders Say

No independently sourced founder testimonials found at this time.

## Sources

[^1]: TechCrunch, "Orbio raises $21 million to automate hiring and onboarding for frontline workers," June 14, 2026. https://techcrunch.com/2026/06/14/orbio-raises-21-million-to-automate-hiring-and-onboarding-for-frontline-workers/
[^2]: Business Review Live, "Orbio raises $21 Mn Series A to transform frontline workforce management with AI," June 2026. https://businessreviewlive.com/orbio-raises-21-mn-series-a-to-transform-frontline-workforce-management-with-ai/
[^3]: Tech.eu, "Orbio raises $21M Series A to bring AI workforce management to the world's frontline workers," June 15, 2026. https://tech.eu/2026/06/15/orbio-raises-21m-series-a-to-bring-ai-workforce-management-to-the-worlds-frontline-workers/
