---
name: "Intropy"
slug: intropy
type: startup
website: "https://www.intropy.ai"
location: "London, United Kingdom"
status_company: active
sector: [ai, supply-chain, spare-parts, industrial, enterprise-software]
stage_latest: "Seed"
total_raised: "€9.5M"
investors: []
firms:
  - slug: general-catalyst
    round: seed
    year: 2026
    date: 2026-07-30
  - slug: quiet-capital
    round: seed
    year: 2026
    date: 2026-07-30
status: published
last_researched: 2026-07-30
---

## About

Intropy is a London-based AI-native platform automating inventory, pricing, and other critical decisions for spare parts businesses [^1][^2]. The company reports that its technology has processed more than €8.7 billion in parts demand and that its customers have achieved returns on investment exceeding 10x [^1][^2]. On 2026-07-30, Intropy announced a €9.5 million seed round (approximately $11 million) led by Felix Capital, with participation from Quiet Capital and existing investors General Catalyst and firstminute capital [^1][^2]. Intropy plans to use the capital to accelerate product development, grow its engineering and machine learning teams, and open a New York office [^1][^2].

*Disclaimer: This is a lean profile built from initial round coverage and may be expanded as more information becomes available.*

## Funding History

| Date | Round | Amount | Lead | Co-investors |
|------|-------|--------|------|--------------|
| 2026-07-30 | Seed | €9.5M | Felix Capital | Quiet Capital, General Catalyst, firstminute capital [^1][^2] |

## What Investors Say

No independently sourced investor commentary quoted at press time.

## What Founders Say

No independently sourced founder quotes at press time.

## Sources

[^1]: EU-Startups, "London-based Intropy raises €9.5 million to build the AI-native operating system for the spare parts industry," July 30, 2026. Accessed July 30, 2026. https://www.eu-startups.com/2026/07/london-based-intropy-raises-e9-5-million-to-build-the-ai-native-operating-system-for-the-spare-parts-industry/
[^2]: Tech Funding News, "Oxford physicist's Intropy lands $11M to bring AI automation to the spare parts industry," July 30, 2026. Accessed July 30, 2026. https://techfundingnews.com/intropy-11m-seed-felix-capital-ai-spare-parts/
