---
name: Elorian
slug: elorian
type: startup
website: "https://elorian.ai"
location: Palo Alto, California
founded: 2026
status_company: active
sector:
- AI
- robotics
- computer-vision
stage_latest: Seed
total_raised: $55M
firms:
- slug: menlo-ventures
  round: seed
  year: 2026
- slug: altimeter-capital
  round: seed
  year: 2026
investors: []
founders:
- name: Andrew Dai
  role: Co-Founder
- name: Yinfei Yang
  role: Co-Founder
status: published
last_researched: 2026-07-17
---

## About

Elorian is a Palo Alto-based AI research lab and product studio focused on visual reasoning, founded by former Google DeepMind researcher Andrew Dai and former Apple AI researcher Yinfei Yang [^1][^2]. The company emerged from stealth on April 9, 2026 at a $300 million valuation [^1]. Elorian builds purpose-built visual reasoning models for robotics, aerospace, medicine, and manufacturing [^2].

## Funding History

| Date | Round | Amount | Lead | Co-investors |
|------|-------|--------|------|--------------|
| 2026-04-09 | Seed | $55M | Striker Ventures, Menlo Ventures, Altimeter | NVIDIA, Jeff Dean [^1][^2][^3] |

## What Investors Say

No verified investor quotes available at this time.

## What Founders Say

No verified founder quotes available at this time.

## Sources

[^1]: Bloomberg / Techmeme, "Ex-Google DeepMind Researchers Debut Startup Called Elorian Focused on Visual AI," April 9, 2026. https://www.techmeme.com/260409/p24
[^2]: Ventureburn, "Elorian Raises $55M for Visual Reasoning AI Growth," April 2026. https://ventureburn.com/elorian-raises-55m-visual-reasoning-ai/
[^3]: TechCrunch, "How a former DeepMind researcher raised at a $300M pre-seed valuation before launching a product," July 16, 2026, accessed July 2026. https://techcrunch.com/2026/07/16/how-a-former-deepmind-researcher-raised-at-a-300m-pre-seed-valuation-before-launching-a-product/
