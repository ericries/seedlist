---
firms:
- date: 2026-04-16
  round: series-c
  slug: khosla-ventures
  year: 2026
- round: series-b
  slug: nea
  year: 2025
- round: series-b
  slug: sequoia-capital
  year: 2025
- round: series-a
  slug: sequoia-capital
  year: 2024
- date: 2026-04-16
  round: series-c
  slug: insight-partners
  year: 2026
founded: 2023
founders:
- name: Matan Grinberg
  role: Co-Founder & CEO
- name: Eno Reyes
  role: Co-Founder
investors:
- round: series-c
  slug: keith-rabois
  year: 2026
- round: Seed
  slug: shaun-maguire
  year: 2023
last_researched: 2026-04-19
location: San Francisco, California
name: Factory
sector:
- AI
- developer-tools
- enterprise
slug: factory
stage_latest: Series C
status: published
status_company: active
total_raised: $220M+
type: startup
website: https://factory.ai
---

## About

Factory is a San Francisco-based startup building AI coding agents ("Droids") for enterprise software development teams [^1][^2]. Founded in 2023 by Matan Grinberg and Eno Reyes, the company reached a $1.5 billion valuation with its Series C [^1][^2]. Factory's AI agents are used daily by hundreds of thousands of developers at enterprises including Nvidia, Adobe, EY, Palo Alto Networks, Adyen, MongoDB, Bayer, Zapier, Clari, Bilt, Profound, and Rogo [^1]. The platform extends beyond code generation into testing, review, documentation, and deployment [^1]. The company has doubled revenue month-over-month for six consecutive months [^2].

## Funding History

| Date | Round | Amount | Lead | Co-investors |
|------|-------|--------|------|--------------|
| 2026-04-16 | Series C | $150M | Khosla Ventures | Sequoia Capital, Blackstone, Insight Partners, Evantic Capital, 20VC, NEA, Mantis VC [^1][^2] |
| 2025-09 | Series B | $50M | NEA | Sequoia Capital, J.P. Morgan, Nvidia, Abstract Ventures, Mantis Ventures [^3] |
| 2024 | Series A | $15M | Sequoia Capital | Lux Capital, Mantis VC [^2] |
| 2023 | Seed | $5M | — | — [^2] |

## What Investors Say

Keith Rabois of Khosla Ventures joined Factory's board as part of the Series C round [^1].

## What Founders Say

No verified founder quotes available at this time.

## Sources

[^1]: TechCrunch, "Factory hits $1.5B valuation to build AI coding for enterprises," April 16, 2026. https://techcrunch.com/2026/04/16/factory-hits-1-5b-valuation-to-build-ai-coding-for-enterprises/
[^2]: Factory Blog, "Factory raises $150M Series C," April 16, 2026. https://factory.ai/news/series-c
[^3]: BusinessWire, "Factory Unleashes the Droids, Raises $50 Million Series B from NEA, Sequoia Capital, NVIDIA, and J.P. Morgan," September 25, 2025. https://www.businesswire.com/news/home/20250925993478/en/Factory-Unleashes-the-Droids-Raises-$50-Million-Series-B-from-NEA-Sequoia-Capital-NVIDIA-and-J.P.-Morgan