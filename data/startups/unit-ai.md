---
name: "UNIT AI"
slug: unit-ai
type: startup
website: "https://unit.ai"
location: "San Francisco, California"
sector: [robotics, warehouse-automation, logistics, physical-ai, ecommerce]
stage_latest: "Growth"
total_raised: "$12M"
investors: []
firms:
  - slug: recursive-ventures
    round: growth
    year: 2026
    date: 2026-07-29
status: published
last_researched: 2026-07-30
---

## About

UNIT AI is a San Francisco-based physical AI company building modular, AI-powered warehouse automation for retailers and third-party logistics (3PL) providers. Its platform is designed to automate direct-to-consumer inventory management and returns without requiring companies to redesign existing facilities, operating in spaces as small as 1,000 square feet [^1][^2]. The company was founded by warehouse automation veterans Guy Glass (previously founder of Caja Robotics, acquired by Fives) and Avihou Barkay (President of Plus One Robotics and GM of Caja Robotics) [^1].

## Funding History

| Date | Round | Amount | Lead | Co-investors |
|------|-------|--------|------|--------------|
| 2026-07-29 | Growth | $12M | Prologis Ventures, Dynamo Ventures, Ground Up Ventures | eGateway Capital, Recursive Ventures, Think + Ventures, ZEP Fund, Crosscourt [^1][^2] |

The round stage was not disclosed in the announcement [^1]. UNIT AI will use the capital to accelerate commercial deployments across North America [^1].

## What Investors Say

No independently sourced investor commentary found beyond the funding announcement.

## What Founders Say

No independently sourced founder testimonials found beyond the funding announcement.

## Sources

[^1]: "UNIT AI Raises $12 Million to Scale AI-Powered Ecommerce Fulfillment and Returns as Customer Demand Accelerates," PR Newswire, July 29, 2026. Accessed July 2026. https://www.prnewswire.com/news-releases/unit-ai-raises-12-million-to-scale-ai-powered-ecommerce-fulfillment-and-returns-as-customer-demand-accelerates-302837792.html
[^2]: "Venture Capital & Startup Funding Roundup, July 29, 2026: Battery Ventures, Elephant, Thomson Reuters Ventures, Y Combinator & More," Tech Startups, July 29, 2026. Accessed July 2026. https://techstartups.com/2026/07/29/venture-capital-startup-funding-roundup-july-29-2026-more/
