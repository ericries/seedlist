---
firms:
- round: seed
  slug: y-combinator
  year: 2023
- round: series-a
  slug: kleiner-perkins
  year: 2026
- date: 2026-04-27
  round: series-b
  slug: general-catalyst
  year: 2026
founded: 2023
investors: []
last_researched: 2026-04-30
location: New York, NY
name: Avoca
sector:
- ai
- vertical-saas
- services-economy
- voice-ai
- ai-agents
slug: avoca
stage_latest: Series B
status: published
status_company: active
total_raised: $125M+
type: startup
website: https://www.avoca.ai
---

## About

Avoca is a New York-based AI workforce platform for service businesses, founded in 2023 by Tyson Chen and Apurva Shrivastava [^1][^2]. The company builds AI agents that handle chat, email, voice calls, and SMS across the customer journey for HVAC, plumbing, automotive, moving, and other home-services operators [^1]. The April 27, 2026 announcement disclosed $125M+ raised across Seed, Series A, and Series B at a $1B valuation, with customers including Turnpoint, 1-800-GOT-JUNK?, Goettl, Sila Services, and H.L. Bowman; Avoca surpassed eight figures in ARR in 2025 and is on track to book $1B in jobs in 2026 [^1][^3].

## Funding History

| Date | Round | Amount | Lead | Co-investors |
|------|-------|--------|------|--------------|
| 2023 | Seed (YC W23) | Undisclosed | Y Combinator | [^1][^4] |
| 2026 | Series A | Undisclosed | Kleiner Perkins | Amplify Partners, Nexus Venture Partners [^1][^3] |
| 2026-04-27 | Series B | Undisclosed (part of $125M+ total) | Meritech, General Catalyst | Kleiner Perkins, Amplify Partners, Nexus Venture Partners [^1][^2] |

## What Investors Say

Alex Clayton, Meritech Capital: "Three years ago, AI voice for home services wasn't a category. Avoca created it" [^1].

Vedant Suri, General Catalyst: "Avoca is building an AI workforce for the people who keep America running" [^1].

Leigh Marie Braswell, Kleiner Perkins: "They're applying AI where execution is the real bottleneck" [^1].

## What Founders Say

Tyson Chen, co-founder: "When a home service business misses a phone call, that could be a $30,000, $40,000 HVAC install they're missing" [^2].

## Sources

[^1]: PR Newswire, "Avoca Raises $125M+ at $1B Valuation to Power America's Services Economy With AI," April 27, 2026. https://www.prnewswire.com/news-releases/avoca-raises-125m-at-1b-valuation-to-power-americas-services-economy-with-ai-302753962.html
[^2]: Fortune, "How a chance encounter in Texas sparked a $1 billion Kleiner Perkins-backed AI startup," April 27, 2026. https://fortune.com/2026/04/27/avoca-ai-agents-missed-calls-hvac-plumbing-roofing-kleiner-perkins-chen-shrivastava-braswell/
[^3]: Avoca, "Why We Are Building — Avoca," April 27, 2026. https://www.avoca.ai/blog/avoca-raises-125m-series-b-1b-valuation
[^4]: "Avoca: AI-powered Sales Agent for Service-Based Industries," Y Combinator company directory. https://www.ycombinator.com/companies/avoca. Accessed April 2026. (Listed as Winter 2023 batch.)