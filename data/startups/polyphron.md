---
name: "Polyphron"
slug: polyphron
type: startup
website: "https://polyphron.com/"
location: "New York, NY"
sector:
  - biotech
  - ai
  - drug-discovery
  - tissue-engineering
stage_latest: "Seed"
total_raised: "$20M (2026 seed)"
investors: []
firms:
  - slug: quiet-capital
    round: seed
    year: 2026
    date: 2026-09-17
founders:
  - name: "Matt Osman"
    role: "CEO & Co-Founder"
  - name: "Fabio Boniolo"
    role: "CSO & Co-Founder"
  - name: "Vinh Q. Tran"
    role: "Chief AI Scientist & Co-Founder"
  - name: "George Pilitsis"
    role: "COO & Co-Founder"
status: published
last_researched: 2026-09-18
---

## About

Polyphron is a New York–based biotech developing artificial human biology by combining physical tissue manufacturing with AI simulation, producing donor-diverse human tissues that serve as a verification substrate for testing biological interventions at scale [^1][^2]. The company's stated aim is to close the gap between AI-generated interventions and the slower biology required to validate them [^1]. Its founding team comes out of Google DeepMind, the Broad Institute and Harvard Medical School, Ginkgo Bioworks, and prior AI startup exits [^1].

## Funding History

| Date | Round | Amount | Lead | Co-investors |
|------|-------|--------|------|--------------|
| 2026-09-17 | Seed | $20M | Quiet Capital | Gradient, Haystack, Compound [^2][^3][^4] |

## What Investors Say

No independently sourced investor quotes beyond announcement boilerplate at time of profile creation.

## What Founders Say

No independently sourced founder testimonials found at time of profile creation.

## Sources

[^1]: Polyphron company website, "Home," accessed September 18, 2026. https://polyphron.com/
[^2]: AlleyWatch, "The AlleyWatch Startup Daily Funding Report: 9/17/2026," September 17, 2026. https://alleywatch.com/2026/09/the-alleywatch-startup-daily-funding-report-9-17-2026/
[^3]: Axios Pro Rata (Dan Primack), deal listing for Polyphron $20M seed, September 17, 2026. https://www.axios.com/newsletters/axios-pro-rata
[^4]: Quiet Capital portfolio page, accessed September 18, 2026. https://quiet.com/portfolio/ — Polyphron is listed among Quiet Capital's active portfolio companies, confirming the lead investor relationship.
