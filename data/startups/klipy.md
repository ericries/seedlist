---
name: "KLIPY"
slug: klipy
type: startup
website: "https://klipy.com"
location: "San Francisco, California"
status_company: active
sector: [ai, developer-tools, consumer, media]
stage_latest: "Seed"
total_raised: "$3.8M"
investors: []
firms: []
founders:
  - name: "Givi Beridze"
    role: "CEO & Co-Founder"
status: published
last_researched: 2026-06-03
---

## About

KLIPY is a San Francisco-based provider of an API platform for expressive media — GIFs, memes, stickers, clips, and AI-generated media — that developers can integrate into social and creative products [^1][^2][^3]. The company says its customers include BeReal, Baidu, Canva, Figma, and Microsoft SwiftKey, and its tools also give creators, brands, and rights holders ways to distribute, manage, and control content [^1][^2].

On June 17, 2026, KLIPY announced a $3.8 million round and disclosed that it had joined Google's AI Futures Fund [^1][^2][^3].

## Funding History

| Date | Round | Amount | Lead | Co-investors |
|---|---|---|---|---|
| 2026-06-17 | Seed | $3.8M | Google AI Futures Fund | I2BF, Silvercircle VC, Sturgeon Capital, Red Swan Ventures, Intuition Ventures, Yash Patel (Capra Ventures), Zaza Pachulia [^1][^2][^3] |

## What Investors Say

Jonathan Silber, Google AI Futures Fund: "KLIPY is building the intelligent, trusted expression layer that the next generation of apps will rely on" [^1].

## What Founders Say

Givi Beridze, CEO: "People are expressing themselves through memes, GIFs, and short-form content more than ever, especially Gen Z and Gen Alpha" [^1].

Frank Nawabi, Co-Founder: "KLIPY is building a next-generation 'expression layer' for the AI era that combines search, AI, and monetization into a unified API platform" [^1].

## Sources

[^1]: PR Newswire, "KLIPY raises $3.8M from investors and joins Google AI Futures Fund to build the expression layer for GIFs, memes, stickers, clips, and AI media," June 17, 2026. Accessed June 2026. https://www.prnewswire.com/news-releases/klipy-raises-3-8m-from-investors-and-joins-google-ai-futures-fund-to-build-the-expression-layer-for-gifs-memes-stickers-clips-and-ai-media-302802594.html

[^2]: Finsmes, "KLIPY Raises $3.8M in Funding," June 2026. Accessed June 2026. https://www.finsmes.com/2026/06/klipy-raises-3-8m-in-funding.html

[^3]: The SaaS News, "KLIPY Secures $3.8M in Funding," June 2026. Accessed June 2026. https://www.thesaasnews.com/news/klipy-secures-3-8m-in-funding/
