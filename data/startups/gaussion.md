---
name: "Gaussion"
slug: gaussion
type: startup
website: "https://gaussion.com"
location: "London, UK"
founded: 2022
status_company: active
sector: [climate, deeptech, batteries, energy, hardware]
stage_latest: "Series B"
total_raised: "€38M+"
investors: []
firms:
  - slug: future-ventures
    round: series-b
    year: 2026
    date: 2026-06-29
founders:
  - name: "Tom Heenan"
    role: "CEO & Co-Founder"
  - name: "Chun Tan"
    role: "Co-Founder"
status: published
last_researched: 2026-06-29
---

## About

Gaussion is a London-based deeptech battery company founded in 2022 as a spinout from University College London and the Faraday Institution [^1][^2]. The company develops MagLiB, a magnetic PCB retrofit system, and Aeon, a proprietary control chip and AI-enabled over-the-air intelligence platform [^1]. Gaussion's technology applies an external magnetic field to enable rapid charging and to improve the performance and lifespan of lithium-ion battery systems without changing battery chemistry or design [^1][^2]. Applications range from EV and drone charging to powering data centres [^1]. In October 2025 the company moved into a 40,000 sq ft facility in central London that it describes as one of the largest battery R&D locations in Europe [^1].

## Funding History

| Date | Round | Amount | Lead | Co-investors |
|------|-------|--------|------|--------------|
| 2026-06-29 | Series B | €24.5M | BGF, AlbionVC | Autotech Ventures, UCL Technology Fund, DN Capital, Future Ventures [^1][^2][^3] |

The Series B brings total funding to over €38M ($44M) [^1][^2][^3]. Future Ventures, led by Steve Jurvetson, joined as a new backer [^1][^2][^3].

## What Investors Say

Dennis Atkinson, co-Head of Early Stage at BGF: "Gaussion has made huge strides validating its technology and scaling operations" [^1].

Sebastian Hunte, Investment Director at AlbionVC: "Batteries underpin the modern world...Gaussion has created a new layer in the battery stack" [^1].

## What Founders Say

CEO Tom Heenan: "The race everyone's watching is about computer chips — but the real prize is the battery" [^1][^2].

CEO Tom Heenan on the underlying physics: "If you apply a magnetic field to that movement, you can make it more efficient" [^3].

## Sources

[^1]: BeBeez International, "From charging EVs and drones to powering data centres: London-based Gaussion raises €24.5 million," June 29, 2026. Accessed June 2026. https://bebeez.eu/2026/06/29/from-charging-evs-and-drones-to-powering-data-centres-london-based-gaussion-raises-e24-5-million/
[^2]: Tech Funding News, "Early Tesla and SpaceX investor joins $28M raise for London startup addressing battery limitations in AI," accessed June 2026. https://techfundingnews.com/gaussion-28m-tesla-investor-battery-startup/
[^3]: AOL (syndicated from The Daily Telegraph), "Elon Musk ally backs British electric car battery start-up," June 2026. Accessed June 2026. https://www.aol.com/articles/elon-musk-ally-backs-british-095307000.html
