---
firms:
- round: Series B
  slug: insight-partners
  year: 2026
- round: Series B
  slug: cyberstarts
  year: 2026
- round: Series B
  slug: index-ventures
  year: 2026
founded: 2023
founders:
- name: Israel Duanis
  role: CEO & Co-Founder
- name: Niv Goldenberg
  role: Co-Founder
investors:
- round: Seed
  slug: hila-zigman
  year: 2024
- round: Seed
  slug: lior-simon
  year: 2024
- round: Early
  slug: shardul-shah
  year: 2022
- round: '2024'
  slug: gili-raanan
  year: 2024
last_researched: 2026-04-02
location: New York, NY
name: Linx Security
sector:
- cybersecurity
- identity-security
- enterprise-software
slug: linx-security
stage_latest: Series B
status: published
status_company: active
total_raised: $83M
type: startup
website: https://www.linx.security
---

## About

Linx Security is a 100-person identity security startup founded in 2023 and headquartered in New York [^1]. The company provides an AI-native identity security and governance platform that monitors and controls all types of identities — human, non-human, and AI agents — across enterprises [^1]. Linx delivers unified visibility, continuous risk detection, and autonomous remediation, and has signed multimillion-dollar contracts with banks, healthcare companies, and Fortune 500 firms governing millions of identities globally [^1]. The company recently introduced Linx Autopilot, an autonomous AI agent for identity governance [^1].

## Funding History

| Date | Round | Amount | Lead | Co-investors |
|------|-------|--------|------|--------------|
| 2026-03-31 | Series B | $50M | Insight Partners | Cyberstarts, Index Ventures [^1][^2][^3] |

Total raised to date: $83M [^1].

## What Investors Say

No verified investor quotes available at this time.

## What Founders Say

No verified founder quotes available at this time.

## Sources

[^1]: Linx Security Blog, "Linx Security Raises $50M Series B," March 31, 2026. Accessed May 2026. https://www.linx.security/blog/linx-security-raises-50m-series-b
[^2]: SiliconANGLE, "Linx Security raises $50M in new funding to grow identity and access platform," March 31, 2026. https://siliconangle.com/2026/03/31/linx-security-raises-50m-new-funding-grow-identity-access-platform/
[^3]: Insight Partners, "Linx Security Raises $50M Series B as Identity Becomes Security's Biggest Failure Point," March 31, 2026. Accessed May 2026. https://www.insightpartners.com/ideas/linx-security-raises-50m-series-b-as-identity-becomes-securitys-biggest-failure-point/