---
name: INXY
slug: inxy
type: startup
website: "https://www.inxy.com"
location: Cyprus
founded: 2024
status_company: active
sector:
- fintech
- payments
- stablecoins
- cross-border-payments
stage_latest: Seed
total_raised: $7M
firms:
- slug: flashpoint
  round: seed
  year: 2026
founders:
- name: Ruslan Zholik
  role: Founder & CEO
status: published
last_researched: 2026-05-26
---

## About

INXY is a Cyprus-based, EU-authorised cross-border payment infrastructure provider that bridges digital assets with traditional payment systems [^1]. The platform enables businesses to accept stablecoins, convert them to fiat or other stablecoins, and process international payouts [^1]. INXY serves 100+ global businesses including advertising networks, freelance platforms, gaming studios, and SaaS companies, processing over $2 billion in annual transaction volume with 500% year-on-year growth [^1].

## Funding History

| Date | Round | Amount | Lead | Co-investors |
|------|-------|--------|------|--------------|
| 2025-02-13 | Seed (initial) | $3M | Flashpoint VC | — [^1][^3] |
| 2026-04-06 | Seed (extension) | $4M | Flashpoint VC | — [^1][^2] |

## What Investors Say

No verified investor quotes available at this time.

## What Founders Say

No verified founder quotes available at this time.

## Sources

[^1]: Cyprus Mail, "INXY raises $7m to expand global payments platform," April 6, 2026. https://cyprus-mail.com/2026/04/06/inxy-raises-7m-to-expand-global-payments-platform
[^2]: Finextra, "Stablecoin startup Inxy raises $4 million," accessed April 2026. https://www.finextra.com/pressarticle/109383/stablecoin-startup-inxy-raises-4-million
[^3]: Cyprus Mail, "Cyprus-based INXY Payments secures $3 million from Flashpoint VC," February 13, 2025. Accessed May 2026. https://cyprus-mail.com/2025/02/13/cyprus-based-inxy-payments-secures-3-million-from-flashpoint-vc
