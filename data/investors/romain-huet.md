---
name: "Romain Huet"
slug: romain-huet
type: individual
firm: ""
role: "Head of Developer Experience, OpenAI; Angel Investor"
location: "San Francisco, California"
stage_focus: [seed, series-a]
sector_focus: [developer-tools, ai, devtools, saas, infrastructure]
check_size: "Undisclosed"
last_verified_investment:
  date: 2026-05-12
  company: "White Circle"
  round: "Seed"
social:
  twitter: "@romainhuet"
  linkedin: "linkedin.com/in/romainhuet"
status: flagged
review_notes: |
  - 5 angel investments independently verified via primary press sources (Lago, PromptLayer, Granola, Swarmia, White Circle). At the 5-investment threshold for cautious inferred-thesis analysis.
  - Check size not disclosed in any cited source; left as "Undisclosed." Confirmed unresolvable after 2026-06 sweep — no public statement of check range surfaced.
  - No independently sourced founder quotes about Huet's involvement as an investor found; "What Founders Say" notes the gap rather than padding.
  - Primary identity is operator (OpenAI DevEx, ex-Stripe). Angel record is real but narrow.
last_researched: 2026-06-03
---

## Background

Romain Huet is a French entrepreneur and engineer based in San Francisco who has spent his career building developer platforms at consumer-scale technology companies [^1]. He joined OpenAI in September 2023 to lead Developer Experience, where he oversees product and developer relations for the OpenAI platform [^2][^1].

Before OpenAI, Huet spent more than five years at Stripe, which he joined in November 2015 from Twitter to lead global developer relations [^3]. He went on to lead product for Stripe's developer platform and co-created Stripe Sessions, the company's developer-focused user conference [^4]. At Twitter (2013-2015) he was a senior developer advocate working on Fabric and Digits [^3]. Earlier, he was co-founder and CTO of Jolicloud, a Paris-based cloud operating system company [^3][^4].

Huet has been an active angel investor in parallel with his operating career, and was a member of the 9th cohort of First Round Capital's Angel Track program in 2021 [^5].

## Stated Thesis

Huet has not published a formal investment thesis. His public communication consistently emphasizes developer tooling and AI infrastructure. Slush describes his focus as "inspiring and supporting founders and builders to integrate AI" while creating powerful developer tools [^4]. His verified angel checks have all gone into companies serving software developers or technical builders [^6][^7][^8][^9].

## Inferred Thesis

**Sample size: 5 verified angel investments. Small but at the threshold for cautious qualitative analysis.**

Across the five independently verified rounds (Lago, PromptLayer, Granola, Swarmia, White Circle), three clear patterns emerge:

- **Developer, AI infrastructure, and builder tooling without exception.** Every verified investment serves software developers, AI builders, or technical teams: Lago (open-source billing infrastructure for SaaS) [^6], PromptLayer (prompt management and AI app development tooling) [^7], Granola (AI note-taking for technical teams) [^8], Swarmia (software engineering intelligence) [^9], and White Circle (AI control and safety platform for enterprise AI) [^12].
- **Predominantly seed and Series A.** PromptLayer was a $4.8M seed [^7]; Lago combined a $7M seed and $15M Series A [^6]; Swarmia was an $11M Series A [^9]; White Circle was an $11M seed [^12]; Granola's Series B was the latest checked round [^8].
- **Co-investing alongside other developer-platform and AI-platform operators.** In Granola's Series B, Huet appeared alongside Guillermo Rauch (Vercel), Amjad Masad (Replit), Tobi Lutke (Shopify), Karri Saarinen (Linear), Des Traynor (Intercom), Karim Atiyeh (Ramp), and Zach Lloyd (Warp) [^8]. In Swarmia he invested alongside Cal Henderson (ex-Slack CTO) and Alex Plugaru (Gorgias CTO) [^9]. In White Circle, his co-investors included Durk Kingma (Anthropic, ex-OpenAI), Guillaume Lample (Mistral), Thomas Wolf (Hugging Face), Olivier Pomel (Datadog), François Chollet (Keras), Paige Bailey (DeepMind), and David Cramer (Sentry) [^12]. The pattern: operator-angels from the developer-platform and frontier-AI worlds clustering on the same deals.

Geographic spread is transatlantic, with a French throughline: Lago is Paris/UK-based [^6], Swarmia is Finnish [^9], Granola is London-based [^8], White Circle is Paris-based [^12], and PromptLayer is US-based [^7]. This is consistent with Huet's own French background and his SF base.

Notable gap: no verified consumer, fintech-payments, or hardware investments in the surfaced record despite his Stripe payments background. His angel checks track his current AI/developer-tooling vantage point at OpenAI rather than his prior fintech operating history.

## Portfolio

| Company | Year | Stage | Source |
|---|---|---|---|
| Lago | 2024-03-14 | Series A ($15M; combined with prior $7M seed) | [^6] |
| Granola | 2025-05-14 | Series B ($43M, led by NFDG) | [^8] |
| PromptLayer | 2025-02-07 | Seed ($4.8M) | [^7] |
| Swarmia | 2025-06-18 | Series A ($11M) | [^9] |
| White Circle | 2026-05-12 | Seed ($11M) | [^12] |

Only five investments could be independently verified via primary press sources. Aggregator sites list additional companies, but those entries were not corroborated by a contemporaneous press source citing Huet by name, so they are not included here.

## In Their Own Words

On joining OpenAI (LinkedIn, September 2023):

> "Excited to share I've joined OpenAI to lead Developer Experience! We're only scratching the surface of what this foundational technology can do. Thrilled to be on the team, eager to empower developers as they build on our platform—let's shape the future of software together!" [^10]

On the design of OpenAI's Responses API, Latent Space podcast, March 11, 2025:

> "Chat completion is definitely here to stay, you know, it's a bare metal API we've had for quite some time." [^11]

> "It was optimized for a pre-multi-modality world... now with these agentic workflows, we noticed that developers and companies want to build longer horizon tasks." [^11]

On OpenAI's Agents SDK and multi-agent design, same episode:

> "Instead of having one agent to rule them all and you stuff a lot of tool calls in there that can be hard to monitor, now you have the tools you need to separate the logic." [^11]

On joining Stripe (TechCrunch, November 4, 2015):

> "I'm incredibly excited to announce I'll be joining the phenomenal team at Stripe in San Francisco to lead developer relations!" [^3]

## What Founders Say

No independently sourced founder testimonials about Huet's involvement as an angel investor were found in primary press, founder blog posts, or podcast transcripts surfaced during this research. Founders of his portfolio companies (Lago, PromptLayer, Granola, Swarmia) acknowledge his participation in their rounds via standard announcement language, but no founder has publicly described the working relationship in a quotable way that meets the project's standard for this section.

## Sources

[^1]: Romain Huet, LinkedIn profile, accessed June 2026. https://www.linkedin.com/in/romainhuet/
[^2]: Romain Huet, "Head of Developer Experience at OpenAI," The Org profile, accessed June 2026. https://theorg.com/org/openai/org-chart/romain-huet
[^3]: Ingrid Lunden, "Stripe Hires Away Twitter's Romain Huet To Lead Global Developer Relations," TechCrunch, November 4, 2015. https://techcrunch.com/2015/11/04/stripe-hires-away-twitters-romain-huet-to-lead-global-developer-relations/
[^4]: Romain Huet speaker bio, Slush, accessed June 2026. https://slush.org/program/romain-huet
[^5]: First Round Capital, "Angel Track" program page, accessed June 2026. https://www.firstround.com/angel-track
[^6]: Romain Dillet, "Lago, a Paris-based open source billing platform, banks $22M," TechCrunch, March 14, 2024. https://techcrunch.com/2024/03/14/lago-a-paris-based-open-source-billing-platform-banks-22m/
[^7]: "PromptLayer is building tools to put non-techies in the driver's seat of AI app development," TechCrunch, February 7, 2025. https://techcrunch.com/2025/02/07/promptlayer-is-building-tools-to-put-non-techies-in-the-drivers-seat-of-ai-app-development/
[^8]: "Granola Launches AI Workspace for Teams and Raises $43M Series B," BusinessWire / TheSaaSNews, May 14, 2025. https://www.thesaasnews.com/news/granola-raises-43-million-in-series-b/
[^9]: Swarmia blog, "We raised an $11M Series A to take software engineering intelligence mainstream," June 18, 2025. https://www.swarmia.com/blog/series-a-funding/
[^10]: Romain Huet, "Excited to share I've joined OpenAI to lead Developer Experience!" LinkedIn post, September 2023. https://www.linkedin.com/posts/romainhuet_excited-to-share-ive-joined-openai-to-lead-activity-7110687027699879936-sfvY
[^11]: swyx and Alessio, "The new OpenAI Agents Platform" with Romain Huet and Nikunj Handa, Latent Space podcast, March 11, 2025. https://www.latent.space/p/openai-agents-platform
[^12]: Cate Lawrence, "White Circle lands $11M to help companies secure AI systems," Tech.eu, May 12, 2026. https://tech.eu/2026/05/12/white-circle-lands-11m-to-help-companies-secure-ai-systems/ ; Sharon Goldman, "Exclusive: White Circle raises $11 million to stop AI models from going rogue," Fortune, May 12, 2026. https://fortune.com/2026/05/12/exclusive-white-circle-raises-11-million-to-stop-ai-models-from-going-rogue-in-the-workplace/ ; "White Circle Raises $11 Million for AI Control Platform," SecurityWeek, May 12, 2026. https://www.securityweek.com/white-circle-raises-11-million-for-ai-control-platform/
