...
def main():
    ...
    # Preserve curated_collections from existing file (not generated by this script)
    if DATA_OUT.exists():
        try:
            existing = json.loads(DATA_OUT.read_text())
            if "curated_collections" in existing:
                result["curated_collections"] = existing["curated_collections"]
